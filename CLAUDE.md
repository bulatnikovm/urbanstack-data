...
Поточний стан **v1.0**: 13 Product, 7 Financial, 9 Operational = 29 метрик. Operational-розділ заповнено 2026-07-09 за результатами аудиту (`looker_extracted/operational/AUDIT.md`) — включає 4 записи `Known issue` (словник категорій заявок, хардкод "Севен", навантаження на юніт і статуси заявок без фільтра деактивованих будинків) і 1 `Потребує узгодження` (3-тя паралельна confirmed/active-методика). Реєстр v1.0 закриває план Фази 1.
