...
Поточний стан **v1.1**: 13 Product, 7 Financial, 15 Operational = 35 метрик. Operational-розділ спочатку заповнили грубо (v1.0, 9 рядків по файлах), потім Микита надав PDF-експорт дашборду (4 сторінки) — прив'язку перевірено по точному збігу колонок (як і для продуктового), розділ переписано до 15 метрик рівня "один елемент дашборду" (v1.1). Метод і повна таблиця — `looker_extracted/operational/_index.md`. 5 з 18 Looker-джерел лишаються непідтвердженими (можливо legacy/drill-down) — там же, розділ "Відкриті питання".
