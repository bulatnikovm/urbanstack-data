...
with flagged as (
    ...
    where f.text_length >= 60          -- короткі заявки збігаються випадково
      ...
      -- Батьківська групова заявка — текст УК, а не мешканця; у детекторі
      -- кампаній вона створювала б синхронність там, де її немає. До
      -- 2026-08-26 відсіювалась сама (не мала complex_id), тепер явно.
