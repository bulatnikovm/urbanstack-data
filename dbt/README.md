...
окрема модель `mart_monthly_tags` з тією ж грануляцією + `tag_set` — НАБІР
міток заявки одним значенням («Забудовник|заплановано 30+»), а не рядок на
кожен тег. Кожна заявка тут рівно один раз, і на це стоїть тест
`assert_tag_sets_match_categories` (згортає набір і звіряє лічильники з
`mart_monthly_categories`). Рядок-на-тег, який був до ANA-10, завищував
будь-який зріз і ламав виключення: заявка «Забудовник + заплановано 30+»
лишалась у цифрі «крім Забудовник» через другу мітку. Розбір набору на
окремі теги робить дашборд.
