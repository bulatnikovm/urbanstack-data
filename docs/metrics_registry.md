...
| PROD-013 | Продуктовий | Churn компанії | Частка втрачених юзерів на rolling 2-міс базі | churned_users / rolling_2m_active_base | month (вся компанія) | postgresqldim9000.dm_company_churn_monthly | не бачив на дашборді | — | — | Active | ⚠️ **v0.3:** суперечність — `dm_company_churn_monthly` є активним джерелом даних продуктового репорту в Looker (41 запуск/90д, `looker_extracted/product/passthrough_dm_company_churn_monthly__72a3cb14.sql`), хоча на 5 сторінках поточного PDF-експорту графік не знайдено. Або прибрали з дашборду нещодавно, або прихований елемент — уточнити з Артемом. |
