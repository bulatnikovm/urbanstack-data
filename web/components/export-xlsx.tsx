 ...
 *
 * Три стани: спокій → «Готую…», де крізь іконку тече крапковий візерунок
 * (`.dot-rain` у globals.css) → «Готово» з галочкою. Останній потрібен, бо
 * збережений файл зникає в теку завантажень і на екрані не лишає сліду —
 * без нього незрозуміло, чи клік узагалі спрацював.
 */
 ...
 * візерунок встигав би лише блимнути — це читається як збій рендеру, а не
 * як відповідь на клік. 700 мс — трохи більше за цикл анімації (0,55 с),
 * тобто крапки встигають пройти іконку наскрізь.
 */
  ...
  return (
    ...
    >
      {state === "done" ? (
        ...
        // Дві копії однієї іконки: нижня — пригашений силует, верхня —
        // та сама, але з крапковою маскою, що їде вниз. Разом читається
        // як «вміст пересипається», а не як мигання.
        <span className="dot-rain size-3.5">
          <Download className="dot-rain-ghost size-3.5" />
          <Download className="dot-rain-flow size-3.5" />
        </span>
      ) : (
